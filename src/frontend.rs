...
impl CodeGen {
    ...
    fn gen_expr(&mut self, expr: &Expr) -> ValueId {
        match expr {
            ...
            Expr::Variable(name) => {
                *self.symbols.get(name).expect(&format!("Undefined variable {}", name))
            }
            ...
            Expr::Call(name, args) => {
                let func_id = *self.func_index.get(name).expect(&format!("Unknown function {}", name));
