...
impl Value {
    ...
    pub fn from_int(i: i64) -> Self {
        let bits = QNAN | TAG_INT | ((i as u64) & PAYLOAD_MASK);
        Value(bits)
    }
    ...
    pub fn to_int(self) -> Option<i64> {
        if (self.0 & QNAN) == QNAN && (self.0 & TAG_MASK) == (QNAN | TAG_INT) {
            ...
            Some((i << 16) >> 16) // sign-extend from 48 bits
        } else {
            None
        }
    }
    ...
    /// allocate a string in the given arena and return a Value pointing to it.
    pub fn from_string_in_arena(s: &str, arena: &mut Arena) -> Self {
        // Layout: [length: usize][bytes..]
        let len = s.len();
        let total_size = std::mem::size_of::<usize>() + len;
        // Allocate raw bytes
        let ptr = arena.alloc_slice(&[]); // We'll overwrite with write
        let ptr = ptr as *mut u8;
        unsafe {
            // Write length at the beginning
            *(ptr as *mut usize) = len;
            // Copy string bytes after the length
    ...
    /// retrieve the string from a value that was created with from_string_in_arena.
    /// returns None if not a string or if the pointer is invalid.
